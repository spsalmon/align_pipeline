...
~/.local/bin/micromamba run -n align python conda_lock_patched.py lock -f environment.yml -p linux-64 --micromamba

# 2. Render the explicit single-platform lock -> conda-linux-64.lock
~/.local/bin/micromamba run -n align conda-lock render -k explicit
